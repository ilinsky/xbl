<?
	class cJSObfuscator
	{
		...
		{
			$this->sInput	= $this->sInput . join("", file($sFileName));
		}
	    ...
	    function stripLineFeeds()
	    {
			// strip line feeds
			$this->sInput	= preg_replace('/[\r\n]/', "", $this->sInput);
	    }
	    ...
	    function obfuscateVariables()
	    {
	        ...
	        preg_match_all('/[^a-zA-Z]([abdsnofimcvr][A-Z][a-zA-Z0-9_]+)/', $sData, $aTemp);
...
//			$aValues	= $this->_normalizeArray($aTemp[1]);
			...
			// Debug
			if ($this->debug)
				echo "<b>Processing local variables:</b><br /><pre>";

	        for ($nIndex = count($aValues)-1; $nIndex >= 0; $nIndex--) {
	        	$sReplace	= /*$nIndex < 26 ? chr(97 + $nIndex) :*/ chr(97 + (int) $nIndex / 10) . ($nIndex % 10);
...
//				$sData	= preg_replace(	'/(\W?)' . $aValues[$nIndex] . '(\W?)/',	'$1' . $sReplace . '$2',	$sData);
	    ...
	    function obfuscatePrivates()
	    {
			...
			// Debug
			if ($this->debug)
				echo "<b>Processing private members:</b><br /><pre>";

	        for ($nIndex = count($aValues)-1; $nIndex >= 0; $nIndex--) {
				$sReplace	= "." . chr(97 + (int) $nIndex / 10) . ($nIndex % 10);
	            $sData		= str_replace($aValues[$nIndex], $sReplace, $sData);
		...
		{
			$sData		= $this->sInput;
			$sDataTemp	= str_replace($this->aStrings, array_fill(0, count($this->aStrings), ""), $sData);

			// find "values"
			preg_match_all('/\"([a-z0-9_\-\#\:\/\.]{2,})\"/i', $sDataTemp, $aTempValues);

			// find .properties
			preg_match_all('/\.(\$?[a-z][a-z0-9_]+)/i', $sDataTemp, $aTempProperties);
...
//	        $this->aStrings	= $this->_normalizeArray(array_merge($this->aStrings, $aTempValues[1], $aTempProperties[1]));
			...
			// Debug
			if ($this->debug)
				echo "<b>Processing public properties and string values:</b><br /><pre>";
...
*/
		...
		function obfuscate()
		{
			...
			$aKeyWords	= array_merge($aKeyWords, array("arguments", "this", "window", "document"));
			$aKeyWords	= array_merge($aKeyWords, array("[_[", "]]"));
			$aKeyWords	= array_merge($aKeyWords, array("[]", "{}", "()", "&&", "||", "==", "!!", "!=", "+=", "-=", "++", "--", "<=", ">="));

			// replace js keywords
			for ($nIndex = count($aKeyWords) - 1; $nIndex >= 0; $nIndex--)
				$sData	= str_replace($aKeyWords[$nIndex], ($nIndex % 10) . chr(122 - floor($nIndex/10)), $sData);
